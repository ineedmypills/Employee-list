...
class Main(tk.Frame):
    ...
    def init_main(self):
        ...
        self.tree.column("sal", width=100, anchor=tk.CENTER)
...
class DB:
    def __init__(self):
        ...
        self.conn = sqlite3.connect('CompanyEmployers.db')
        # создание объекта класса cursor, используемый для взаимодействия с БД
        self.c = self.conn.cursor()
        # выполнение запроса к БД
        self.c.execute(
            '''CREATE TABLE IF NOT EXISTS db (id integer primary key, name text, tel text, email text, sal int)''')
